...
VS Code has no public API to enumerate currently open windows, so detection
uses its recently opened workspace storage as an honest proxy: only entries
that still exist on disk are reported, each carrying an explicit warning that
this is recency, not a live window list.
"""
...
class JsonFileWorkspaceStorageReader:
    """Read VS Code's workspace storage JSON file directly from disk."""
...
class VSCodeAdapter(BaseResourceAdapter):
    """Detect reachable recently opened VS Code workspaces."""

    type_name = "vscode"

    def __init__(self, reader: WorkspaceStorageReader | None = None) -> None:
        """Create a VS Code adapter with an injectable storage reader."""
        self._reader = reader or JsonFileWorkspaceStorageReader(default_storage_path())
    ...
    def detect(self, context: CaptureContext) -> list[DetectedResource]:
        ...
        if context.platform is not Platform.MACOS:
            raise UnsupportedPlatformError(
                f"VS Code detection is unsupported on: {context.platform.value}",
    ...
    def capture(self, resource: DetectedResource) -> ResourceSpec:
        """Convert one VS Code finding into a review-required resource."""
        if resource.type_name != self.type_name:
            raise ManifestValidationError(
                f"VS Code adapter cannot capture type: {resource.type_name}",
                details={"type": resource.type_name},
            )
        workspace_path = resource.config.get("workspace_path")
        if not isinstance(workspace_path, str) or not workspace_path:
            raise ManifestValidationError(
                "Detected VS Code workspace is missing its path",
            )
